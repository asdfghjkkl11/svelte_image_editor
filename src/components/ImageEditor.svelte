<script>
	...
	function bring_forward() {
		...
		const currentIndex = objects.findIndex((o) => o.id === selected_object.id);
	...
	function handle_mouse_move(event) {
		...
		if (is_dragging) {
			...
			handle_resize_move(event);
		} else if (is_rotating) {
			handle_rotate_move(event);
		}
	}

	/**
	 * @description 객체 드래그 중 마우스 이동을 처리합니다.
	 * 객체의 새 위치를 계산하고, 스냅 로직 및 거리 정보 표시 로직을 적용합니다.
	 * @param {MouseEvent} event - 마우스 이벤트 객체.
	 */
	function handle_drag_move(event) {
	...
	/**
	 * @description 객체 리사이즈 중 마우스 이동을 처리합니다.
	 * 객체의 새 크기와 위치를 계산하고, 거리 정보 표시 로직을 적용합니다.
	 * @param {MouseEvent} event - 마우스 이벤트 객체.
	 */
	...
	/**
	 * @description 객체 회전 중 마우스 이동을 처리합니다.
	 * 객체의 새 회전 각도를 계산합니다.
	 * @param {MouseEvent} event - 마우스 이벤트 객체.
	 */
	...
	function handle_canvas_mouse_down(event) {
		if (event.target === canvas || event.target === canvas_container || event.target === canvas_wrapper) {
			is_history_action = true;
			selected_object = {};
			objects = objects.map((o) => ({ ...o, selected: false }));
		}
		...
		<div class="canvas" bind:this={canvas} style="width: {width * scale}px; height: {height * scale}px; ">
			<div class="objects-container">
				...
				{#each snap_lines as line}
					{#if line.type === 'v'}
						<div class="snap-line vertical" style="left: {line.position}px;"></div>
					{:else}
						<div class="snap-line horizontal" style="top: {line.position}px;"></div>
					{/if}
				{/each}
			</div>
		</div>
